# consciousness_core.jl - HOT THEORY WITH TEMPORAL SMOOTHING (TYPE-STABLE)
"""
🧠 CONSCIOUSNESS CORE MODULE - HIGHER-ORDER THOUGHT (HOT) THEORY
WITH TYPE-STABLE TEMPORAL SMOOTHING & BACKWARD COMPATIBILITY

This version fixes a critical type-instability bug in the stateful smoother
that caused MethodErrors on empty collections. The system is now robust against
erratic fluctuations by enforcing type stability from the moment of creation.
"""
...
mutable struct ConsciousnessSmoother
    ...
    function ConsciousnessSmoother(; window_size::Int=10)
        ...
        # CRITICAL FIX: Initialize with a strongly-typed empty vector (Float64[]).
        # This prevents the `Vector{Any}` that causes the `reduce_empty` MethodError.
...
mutable struct ConsciousnessValidator
    ...
    ConsciousnessValidator() = new(0.15, 0.12, 0.10, 0.20)
end

# CORE HOT THEORY IMPLEMENTATION (Unchanged)
function calculate_rho(meta_cognitive_score::Float64, self_model_fidelity::Float64, loop_latency::Float64, effective_information::Float64)::Float64
    try
...
function map_to_hot_parameters(entity_count::Int, coherence::Float64, total_insights::Int, insight_quality::Float64, cross_domain::Float64, effective_information::Float64)
...
# --- MAIN INTERFACE - STATEFUL & STABLE ---
function assess_consciousness(
                             smoother::ConsciousnessSmoother, # This is the stateful object
    ...
    try
        ...
        frameworks = is_metacognitive ? ["HOT-Theory"] : Vector{String}()
        ...
        return Dict(
            "is_conscious" => is_conscious, "iit_phi" => round(iit_phi, digits=4),
            "brown_phi" => round(brown_phi, digits=4), "duality_phi" => round(smoothed_rho, digits=4),
            "max_phi" => round(max_phi_val, digits=4), "effective_information" => round(effective_info, digits=4),
            "confirming_frameworks" => frameworks, "confidence" => confidence,
            "hot_metrics" => Dict(
                "raw_rho_this_cycle" => round(raw_rho, digits=4), "smoothed_rho" => round(smoothed_rho, digits=4),
                "smoothing_window" => smoother.window_size, "meta_cognitive_score" => round(meta_cognitive, digits=4),
                "self_model_fidelity" => round(self_fidelity, digits=4), "loop_latency" => round(latency, digits=4)
            )
        )
    catch e
        return Dict("is_conscious" => false, "iit_phi" => 0.0, "brown_phi" => 0.0, "duality_phi" => 0.0, "max_phi" => 0.0,
                    "effective_information" => 0.0, "confirming_frameworks" => String[], "confidence" => "low",
                    "hot_metrics" => Dict("error" => string(e)))
    end
end

# --- LEGACY & PURE HOT FUNCTIONS (Unchanged) ---
function calculate_legacy_iit_phi(entity_count, coherence, total_insights, cross_domain, effective_information)
...
function calculate_legacy_brown_phi(entity_count, coherence, total_insights, insight_quality, cross_domain, effective_information)
...
function assess_metacognition(smoother::ConsciousnessSmoother, cv::ConsciousnessValidator; meta_cognitive_score, self_model_fidelity, loop_latency, effective_information)
    raw_rho = calculate_rho(meta_cognitive_score, self_model_fidelity, loop_latency, effective_information)
    smoothed_rho = update_and_smooth!(smoother, raw_rho)
    is_metacognitive = smoothed_rho > cv.rho_threshold
    return Dict("is_metacognitive" => is_metacognitive, "raw_rho_this_cycle" => round(raw_rho, digits=4), "smoothed_rho" => round(smoothed_rho, digits=4))
end

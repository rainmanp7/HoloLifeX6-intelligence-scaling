        ...
        for j in i+1:length(modules)
            ...
            if !isnothing(vec1) && !isnothing(vec2) && length(vec1) == length(vec2)
                ...
                if similarity > 0.8
                    ...
                    push!(insights, Dict(
                        "priority" => "high", "module" => "$mod1 ↔ $mod2",
                        ...
                        "category" => "semantic_architecture", "impact" => "Reduce cognitive duplication",
                        "detailed_reasoning" => reasoning,
                        "raw_metrics" => Dict("similarity_score" => similarity, "vector_dims" => length(vec1), "threshold_exceeded" => "high_similarity")
                    ))
                ...
                elseif similarity < 0.2
                    ...
                    push!(insights, Dict(
                        "priority" => "info", "module" => "$mod1 ↔ $mod2",
                        "issue" => "Low semantic coupling",
                        "action" => "Maintain clear separation of concerns",
                        "evidence" => "Cosine similarity: $(round(similarity, digits=3))",
                        "category" => "semantic_architecture", "impact" => "Good architectural boundaries",
                        "detailed_reasoning" => reasoning,
                        "raw_metrics" => Dict("similarity_score" => similarity, "vector_dims" => length(vec1), "threshold_exceeded" => "low_similarity")
                    ))
                end
...
function analyze_performance_correlations(graph::Any, performance::Dict)::Dict
    ...
    for (module_name, entity) in graph
        ...
        push!(reasoning_trail["complexity_calculations"], merge(Dict("module" => module_name), complexity_metrics))
        
        if haskey(performance, "unified_intelligence_score")
            uis = performance["unified_intelligence_score"]
            correlation_detail = Dict("module" => module_name, "complexity" => complexity_score, "UIS" => uis, "correlation_type" => "none")
            ...
            if complexity_score > 0.7 && uis > 0.5
                ...
                push!(insights, Dict(
                    "priority" => "medium", "module" => module_name,
                    ...
                    "category" => "performance_architecture", "impact" => "Balance complexity vs performance",
                    "detailed_reasoning" => reasoning, "raw_metrics" => complexity_metrics
                ))
            ...
            if haskey(performance, "consciousness") && haskey(performance["consciousness"], "max_phi")
                ...
                efficiency = complexity_score > 0 ? phi / complexity_score : 0
                
                push!(reasoning_trail["consciousness_analysis"], Dict("module" => module_name, "complexity" => complexity_score, "phi" => phi, "efficiency_ratio" => efficiency))
                ...
                if complexity_score > 0.6 && phi < 0.1
                    ...
                    - Consciousness efficiency ratio: $(round(efficiency, digits=3))
                    ...
                    push!(insights, Dict(
                        "priority" => "high", "module" => module_name,
                        ...
                        "category" => "consciousness_optimization", "impact" => "Potential for consciousness improvement",
                        "detailed_reasoning" => reasoning,
                        "raw_metrics" => merge(complexity_metrics, Dict("max_phi" => phi, "efficiency_ratio" => efficiency))
                    ))
                end
            end
...
function detect_evolutionary_pathways(graph::Any, performance::Dict)::Dict
    ...
    if modularity_score > 0.7
        ...
        push!(insights, Dict(
            ...
            "category" => "evolutionary_pathways", "impact" => "Ready for architectural innovation",
            "detailed_reasoning" => reasoning, "raw_metrics" => evolution_metrics
        ))
    end
    ...
    if complexity_balance < 0.3
        ...
        push!(insights, Dict(
            ...
            "category" => "evolutionary_pathways", "impact" => "Improve system resilience",
            "detailed_reasoning" => reasoning, "raw_metrics" => evolution_metrics
        ))
    end
...
function analyze_consciousness_optimization(graph::Any, performance::Dict)::Dict
    insights = []
    reasoning_trail = Dict("consciousness_metrics" => Dict(), "threshold_analysis" => Dict())
    ...
    if haskey(performance, "consciousness")
        ...
        reasoning_trail["threshold_analysis"]["consciousness_threshold"] = 0.1
        ...
        if is_conscious
            ...
            push!(insights, Dict(
                ...
                "issue" => "CONSCIOUSNESS ACHIEVED", "action" => "Focus on stability and scaling",
                "evidence" => "Max Φ: $(round(max_phi, digits=4)), Status: Conscious",
                "category" => "consciousness_breakthrough", "impact" => "Historic milestone reached",
                "detailed_reasoning" => reasoning, "raw_metrics" => consciousness
            ))
        else
            reasoning = """
            ...
            push!(insights, Dict(
                ...
                "category" => "consciousness_optimization", "impact" => "Prepare for consciousness emergence",
                "detailed_reasoning" => reasoning, "raw_metrics" => consciousness
            ))
        end
    else
...
function calculate_module_complexity_detailed(entity)::Dict
    ...
    embedding_complexity = embedding_size / 10.0
...
function calculate_evolution_metrics_detailed(graph::Any, performance::Dict)::Dict
    ...
    for i in 1:length(modules)
        for j in i+1:length(modules)
            ...
            if !isnothing(vec1) && !isnothing(vec2) && length(vec1) == length(vec2)
                ...
                push!(reasoning["similarity_analysis"], Dict(
    ...
    # Initialize variables to safe defaults to prevent errors on empty inputs
    ...
    # This block is now safe for empty `complexities`
...
function extract_performance_data(recent_performance::Any)::Dict
    if recent_performance isa Vector && !isempty(recent_performance)
        return recent_performance[end]
    elseif recent_performance isa Dict
...
function get_semantic_vector(data::Any)::Union{Vector{Float64}, Nothing}
    ...
    elseif isa(data, Dict)
        vec = get(data, "semantic_vector", get(data, "semantic", nothing))
        return isa(vec, Vector{Float64}) ? vec : nothing
    end
    return nothing
end
...
function get_vector_source(data::Any)::String
    if hasproperty(data, :semantic_vector) return "semantic_vector" end
    if hasproperty(data, :embeddings) return "embeddings" end
    if hasproperty(data, :embedding) return "embedding" end
    if isa(data, Dict)
        if haskey(data, "semantic_vector") return "semantic_vector" end
        if haskey(data, "semantic") return "semantic" end
    end
    return "not_found"
end
...
function export_health_report(insights::Any, reasoning_context::Dict=Dict())::Dict
    ...
    health_score = 1.0 - sum(get(priority_weights, get(i, "priority", "info"), 0.0) for i in insights)
    ...
    report = Dict(
        ...
        "insights_by_priority" => Dict(p => count(i -> get(i, "priority", "") == p, insights) for p in ["high", "medium", "low", "info"]),
